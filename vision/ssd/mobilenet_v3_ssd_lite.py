...
def create_mobilenetv3_ssd_lite(num_classes, width_mult=1.0, is_test=False):
    ...
    source_layer_indexes = [GraphPath(11, 'conv'),19,]

    extras = ModuleList([
        InvertedResidual(288, 512, stride=2, expand_ratio=0.88),
    ...
    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(288 * width_mult), out_channels=4 * 4,
                        kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=288, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
    ...
    classification_headers = ModuleList([
        SeperableConv2d(in_channels=round(288 * width_mult), out_channels=4 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=288, out_channels=6 * num_classes, kernel_size=3, padding=1),
...
def create_mobilenetv3_ssd_lite_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device(0)):
