...
class SqueezeBlock(nn.Module):
    def __init__(self, exp_size, divide=4):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(exp_size, exp_size // divide, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(exp_size // divide),
            nn.ReLU(inplace=True)
        )
        self.conv_ = nn.Sequential(
            nn.Conv2d(exp_size // divide, exp_size, kernel_size=1, stride=1, padding=0, bias=False),
    ...
    def __init__(self, model_mode="SMALL", num_classes=30, multiplier=1.0, dropout_rate=0.0):
        ...
        if model_mode == "LARGE":
            ...
            self.init_conv = nn.Sequential(
                nn.Conv2d(in_channels=3, out_channels=init_conv_out, kernel_size=3, stride=2),
            ...
            layers = [
                ...
                [48, 48, 5, 2, "HS", True, 288],
                [48, 48, 5, 1, "HS", True, 288],
                [48, 48, 5, 1, "HS", True, 288],
            ]
            ...
            # self.frist_conv = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3,stride=2, padding=1)
            ...
            # self.features.append(nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3,stride=2, padding=1))
            ...
            out_conv1_in = _make_divisible(48 * multiplier)
            out_conv1_out = _make_divisible(288 * multiplier)
            self.out_conv1 = nn.Sequential(
                nn.Conv2d(out_conv1_in, out_conv1_out, kernel_size=1, stride=1),
                nn.BatchNorm2d(out_conv1_out),
                h_swish(inplace=True),
            ...
            out_conv2_in = _make_divisible(288 * multiplier)
            out_conv2_out = _make_divisible(256 * multiplier)
            # self.out_conv2 = nn.Sequential(
            #     nn.Conv2d(out_conv2_in, out_conv2_out, kernel_size=1, stride=1),
            #     h_swish(inplace=True),
            #     nn.Dropout(dropout_rate),
